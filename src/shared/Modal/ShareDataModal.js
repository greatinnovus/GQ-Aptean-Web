			...
			<Modal.Body className="appTextColor">
				...
				<div className={classes.colorContainer}>
						...
						<ul className={"list-inline " + classes.scrollList}>
							{
								props.data && Object.keys(props.data).map((dta, i) => {
									console.log(dta)
									const id = (props.type === 'Folder') ? props.data[dta].user_id : props.data[dta].id
									return <li className={"my-2 " + (props.sharedUserId && props.sharedUserId.includes(id) ? 'disabled' : '')} key={i}>
										<span className={"cursorPointer " + (selectData.includes(id) ? classes.projTitleActive : '')} onClick={() => getSelectVal(id)}>{props.data[dta].full_name}</span>
									</li>
								})
							}
						</ul>
					...
					<div className={classes.footerDiv + " float-right"}>
						...
						{/* <Button onClick={props.tryAgain} className={classes.buttonStyle} color="primary" variant="contained">{'Share ' + props.type}</Button>  */}
