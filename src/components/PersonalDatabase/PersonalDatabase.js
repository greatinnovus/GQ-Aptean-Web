    ...
    async function constrainTemplateData(resultCon) {
        ...
        resultCon.map(datas => {
            ...
            if (datas.type == 'DlPhysicalSeqdb') {
                let typeUrl = process.env.REACT_APP_API_URL + url.browseSeqDB.replace('**', datas.id);
                datas.typeContent = 'Physical Sequence Database';
                datas.nameContent = <Fragment>
                    <a href={typeUrl} target="_blank" rel="noreferrer">{datas.description}</a>
