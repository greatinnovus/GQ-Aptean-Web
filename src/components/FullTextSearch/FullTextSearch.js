...
function FullTextSearch() {
  ...
  const getKeyCode = (e) => {
    ...
    if (e.keyCode == 8) {
      ...
      callParseQuery(e.target.textContent, htmlElement);
    } else {
      e.stopPropagation();
    }
  };
  ...
  function replaceStringHtml(value, keyCode, checkLastChar) {
    ...
    let searchTxt = "publication_Date";
    if (
      getCurrentSel &&
      getCurrentSel.focusNode &&
      getCurrentSel.focusNode.parentNode
    ...
    if (rightArrowEvent) {
      console.log("detect arrow");
      console.log(getChildText, "getChildText");
      console.log(pubString, "pubString");
      if (lastChild && lastChild.attributes.length > 0) {
        if (
          getChildText &&
          searchTxt.toLowerCase().includes(getChildText.toLowerCase())
        ) {
      ...
      if (value != "") {
        ...
        console.log(
          lastChild && lastChild.attributes.length,
        ...
        if (lastChild && lastChild.attributes.length > 0) {
          if (getChildClassName == "autoquery") {
            ...
            if (
              ...
              searchTxt
                .toLowerCase()
                .includes(getSelectedNodeText.toLowerCase())
            ) {
              // htmlElement.children[htmlElement.children.length - 1].outerHTML = pubString;
              console.log(pubString, "pubString");
              console.log(pubString.outerHTML, "pubString.outerHTML");
              console.log(getSelectedNodeText, "getSelectedNodeText");
              console.log(
                getSelectedNodeText.length,
                "getSelectedNodeText.length"
              );
              if (getSelectedNodeText.length == 2) {
                let styleAttr = `position:absolute;top:${currOffsetTop}px;left:${currOffsetLeft}px;`;
                var isPubSpanExists = document.getElementsByClassName("pub");
                // Checking If Already Auto Suggest Text updated in DOM
                if (isPubSpanExists.length == 0) {
                  var newSpan = document.createElement("span");
                  ...
                  isPubSpanExists[0].style.cssText = styleAttr;
                }
              }
            } else {
        ...
        <Col md="10" lg="10">
          ...
          <div className="form-group" id="searchDiv">
            ...
            <div
              ...
              onInput={(e) => callParseQuery(e.target.textContent, e)}
              tabIndex="0"
            ></div>
            <div
              className={
